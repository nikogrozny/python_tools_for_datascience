...
pytesseract.tesseract_cmd = r'insert_path_to_tesseract'
path_data = os.getcwd()
...
def acp_freq(cha: AgglomerativeClustering):
    ...
    for axe in range(4):
        ...
        plt.savefig(os.path.join(path_data, f"acp_freq_axes_cha_{2 * axe}_{2 * axe + 1}.png"))

    kmns: KMeans = KMeans(n_clusters=8)
    kmns.fit(proximity)
    Ctr = pca.transform(kmns.cluster_centers_)
    for axe in range(4):
        plt.figure(figsize=(14, 14))
        plt.scatter(XP[:, 2 * axe], XP[:, 2 * axe + 1], edgecolors="none", marker="o", c=kmns.labels_, cmap="tab10",
                    s=120)
        plt.scatter(Ctr[:, 2 * axe], Ctr[:, 2 * axe + 1], c="red", marker="*")
        plt.savefig(os.path.join(path_data, f"acp_freq_axes_km_{2 * axe}_{2 * axe + 1}.png"))
...
def tableau_pour_etiquetage(nb_blocs: int, longueur: int):
    ...
    shuffle(fichiers)
    for nom_fichier in fichiers[:nb_blocs]:
        ...
        if nb_blocs > 2:
            ...
            bloc: int = randrange(0, nb_blocs-1, 1)
            data = data.append({"source": nom_fichier, "text": blocs[bloc], "is_army": np.nan}, ignore_index=True)
    data.to_csv(os.path.join(path_data, "a_classer.csv"), sep=";", encoding="utf-8", index=False)



if __name__ == "__main__":
    # ngram(["communiste", "ordre", "boulanger", "eglise", "guerre"])
    # test_ocr("jo81clean2.jpg")
    ...
    # exemple_geo()
    # tableau_pour_etiquetage(400, 6000)
